...
upper_diagonal_layer = np.diag([2 * tau * 1j / (h ** 2)] * (m - 1), 1)
lower_diagonal_layer = np.diag([2 * tau * 1j / (h ** 2)] * (m - 1), -1)
upper_diagonal_layer_first = np.diag([tau * 1j / (h ** 2)] * (m - 1), 1)
lower_diagonal_layer_first = np.diag([tau * 1j / (h ** 2)] * (m - 1), -1)
...
def layer_matrix_u_f(u_layer, v_layer):
    ...
    val_mat = upper_diagonal_layer + lower_diagonal_layer + main_diagonal
...
def first_layer_matrix_u_f(u_value, v_value):
    ...
    val_mat = upper_diagonal_layer_first + lower_diagonal_layer_first + main_diagonal
